...
class TestATRCalculator(unittest.TestCase):
    ...
    def test_long_position_example(self):
        ...
        result = calculate_atr_trade_parameters(
        ...
        self.assertIn(
            'position_size', result, "Result should contain 'position_size'"
        )
        self.assertIn(
            'stop_loss_percentage', result,
            "Result should contain 'stop_loss_percentage'"
        )
        self.assertAlmostEqual(
            result['position_size'], expected_position_size, places=5
        )
        self.assertAlmostEqual(
            result['stop_loss_percentage'], expected_stop_loss_percentage, places=5
        )
    ...
    def test_short_position_example(self):
        ...
        # Position size calculation is independent of long/short
        expected_position_size = 20.0
        # Stop Loss Distance = ATR * Multiplier = 5.0 * 2.0 = 10.0
        # Stop Loss Percentage = Stop Loss Distance / Entry Price * 100
        #                      = 10.0 / 100.0 * 100 = 10.0%
    ...
    def test_default_parameters_long(self):
        ...
        # Assumes calculate_atr_trade_parameters has defaults for risk_amount
        # and multiplier
        result = calculate_atr_trade_parameters(
            entry_price=entry_price, atr=atr, position_type=position_type
        )
        self.assertIn('position_size', result)
        self.assertIn('stop_loss_percentage', result)
        self.assertAlmostEqual(
    ...
    def test_input_validation_raises_value_error(self):
        ...
        # Test cases: (entry_price, atr, risk_amount, multiplier,
    ...
    def test_invalid_parameter_types_or_values(self):
        ...
        for entry, atr_val, risk, mult, p_type, desc in invalid_param_sets:
            with self.subTest(desc=desc, params=(entry, atr_val, risk, mult, p_type)):
                # Could also be TypeError for wrong types
                with self.assertRaises(ValueError):
                    calculate_atr_trade_parameters(
                        entry_price=entry,
                        atr=atr_val,
