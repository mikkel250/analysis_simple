"""
Market data functionality for the Financial Analysis kernel.

This module provides functions for fetching, processing, and visualizing
financial market data to be used within the custom Jupyter kernel.
"""
...
def plot_price_history(
    ...
    if 'symbol' in data.columns and len(data['symbol'].unique()) > 1:
        for symbol, group in data.groupby('symbol'):
            ax.plot(group.index, group[price_col], label=symbol)
        ax.legend()
    else:
